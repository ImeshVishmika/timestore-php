...
-- Dumping data for table timestore.admin: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `buy_now_cart` (
  ...
  CONSTRAINT `fk_buy_now_product` FOREIGN KEY (`model_id`) REFERENCES `product_has_model` (`model_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.buy_now_cart: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `cart` (
  ...
  CONSTRAINT `fk_cart_product1` FOREIGN KEY (`product_id`) REFERENCES `product_has_model` (`model_id`),
...
-- Dumping data for table timestore.cart: ~0 rows (approximately)
...
-- Dumping data for table timestore.delivery_method: ~3 rows (approximately)
...
-- Dumping data for table timestore.invoice: ~9 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `invoice_items` (
  ...
  `qty` int NOT NULL,
  `invoice_id` int NOT NULL,
  `model_name` varchar(50) CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `model_price` double NOT NULL,
  `model_id` int DEFAULT NULL,
  PRIMARY KEY (`invoice_item_id`) USING BTREE,
  KEY `invoice_items_ibfk_1` (`order_id`) USING BTREE,
  KEY `FK_invoice_items_product_has_model` (`model_id`) USING BTREE,
  KEY `invoice_items_ibfk_2` (`invoice_id`),
  CONSTRAINT `FK_invoice_items_product_has_model` FOREIGN KEY (`model_id`) REFERENCES `product_has_model` (`model_id`),
...
-- Dumping data for table timestore.invoice_items: ~0 rows (approximately)
...
-- Dumping data for table timestore.messages: ~4 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `model` (
  ...
  KEY `FKgyqjs8452aootital0r3qbii0` (`product_id`),
  CONSTRAINT `FKgyqjs8452aootital0r3qbii0` FOREIGN KEY (`product_id`) REFERENCES `product` (`product_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.model: ~0 rows (approximately)
...
-- Dumping data for table timestore.order: ~13 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `order_has_model` (
  ...
  CONSTRAINT `order_has_model_ibfk_1` FOREIGN KEY (`model_id`) REFERENCES `product_has_model` (`model_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `product_has_category` (
  ...
  CONSTRAINT `fk_product_has_category_product1` FOREIGN KEY (`product_id`) REFERENCES `product_has_model` (`model_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.product_has_category: ~2 rows (approximately)
INSERT INTO `product_has_category` (`product_id`, `category_category_id`, `category_id`) VALUES
	(101, 1, 0),
	(102, 2, 0);

-- Dumping structure for table timestore.product_has_model
CREATE TABLE IF NOT EXISTS `product_has_model` (
  `model_id` int NOT NULL AUTO_INCREMENT,
...
-- Dumping data for table timestore.product_has_model: ~33 rows (approximately)
INSERT INTO `product_has_model` (`model_id`, `model`, `product_id`, `price`, `qty`, `added_time`) VALUES
...
CREATE TABLE IF NOT EXISTS `product_img` (
  ...
  CONSTRAINT `fk_product_img_product1` FOREIGN KEY (`model_id`) REFERENCES `product_has_model` (`model_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.product_img: ~31 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `ratings` (
  ...
  CONSTRAINT `fk_product_rating` FOREIGN KEY (`product_id`) REFERENCES `product_has_model` (`model_id`),
  CONSTRAINT `fk_user_rating` FOREIGN KEY (`user_email`) REFERENCES `users` (`email`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.ratings: ~2 rows (approximately)
...
-- Dumping data for table timestore.users: ~4 rows (approximately)
...
-- Dumping data for table timestore.user_address: ~3 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `user_history` (
  ...
  CONSTRAINT `user_product` FOREIGN KEY (`product_id`) REFERENCES `product_has_model` (`model_id`)
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb3;

-- Dumping data for table timestore.user_history: ~4 rows (approximately)
...
-- Dumping data for table timestore.user_img: ~3 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `watchlist` (
  ...
  CONSTRAINT `fk_wishlist_product1` FOREIGN KEY (`product_id`) REFERENCES `product_has_model` (`model_id`),
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `model_data` AS select `brand`.`brand_id` AS `brand_id`,`brand`.`brand_name` AS `brand_name`,`product`.`product_id` AS `product_id`,`product`.`product_name` AS `product_name`,`product_has_model`.`model_id` AS `model_id`,`product_has_model`.`model` AS `model`,`product_has_model`.`price` AS `price`,`product_has_model`.`qty` AS `qty`,`product_has_model`.`added_time` AS `added_time`,`product_img`.`img_path` AS `img_path` from (((`brand` join `product` on((`brand`.`brand_id` = `product`.`brand_id`))) join `product_has_model` on((`product`.`product_id` = `product_has_model`.`product_id`))) join `product_img` on((`product_has_model`.`model_id` = `product_img`.`model_id`))) WITH CASCADED CHECK OPTION;

-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `order_data`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `order_data` AS select `users`.`email` AS `email`,`users`.`fname` AS `fname`,`users`.`lname` AS `lname`,`order`.`order_id` AS `order_id`,`order`.`ordered_date` AS `ordered_date`,`order_has_model`.`qty` AS `order_qty`,`delivery_method`.`delivery_method` AS `delivery_method`,`delivery_method`.`price` AS `delivery_fee`,`order_has_model`.`model_id` AS `model_id`,`product_has_model`.`model` AS `model`,`product_has_model`.`price` AS `price`,`product_img`.`img_path` AS `img_path`,`product`.`product_id` AS `product_id`,`product`.`product_name` AS `product_name`,`brand`.`brand_id` AS `brand_id`,`brand`.`brand_name` AS `brand_name`,`order_status`.`status` AS `status` from ((((((((`order` join `users` on((`order`.`email` = `users`.`email`))) join `order_has_model` on((`order`.`order_id` = `order_has_model`.`order_id`))) join `product_has_model` on((`order_has_model`.`model_id` = `product_has_model`.`model_id`))) join `delivery_method` on((`order`.`delivery_method` = `delivery_method`.`id`))) join `product_img` on((`product_has_model`.`model_id` = `product_img`.`model_id`))) join `product` on((`product_has_model`.`product_id` = `product`.`product_id`))) join `brand` on((`product`.`brand_id` = `brand`.`brand_id`))) join `order_status` on((`order`.`order_status` = `order_status`.`order_status_id`))) WITH CASCADED CHECK OPTION;
